...
class JDEX:
    ...
    def filter_unsatisfiable(self):
        ...
        tot_removed = 0
        self.ui.info(
            f"Starting unsatisfiable class and role removal using {self.config.reasoning.satisfiability.reasoner.upper()}"
        )
        while True:
            ...
            self.ui.info(
                f"Unsatisfiability cleanup step {sc_it_number}: found {len(unsatifiable_classes)} unsatisfiable URI(s)"
            )
            if unsatifiable_classes:
                self.ui.info(f"Unsatisfiable URIs detected: {unsatifiable_classes}")
                self.reasoner.filtering(
                    self.cwd / pc.ONTOLOGY,
                    self.cwd / pc.ONTOLOGY,
                    uris=unsatifiable_classes,
                    verbose=self.config.verbose,
                )
                sc_it_number += 1
                ...
                self.ui.success(
                    f"Unsatisfiability cleanup completed: removed {tot_removed} URI(s) across {sc_it_number} step(s)"
                )
    ...
    def consistency_check(self):
        if self.config.reasoning.consistency.convert_ntriples:
            self.ui.info(
                "Converting the knowledge graph to N-Triples format for consistency checking"
            )
            fg = Graph()
            fg.parse(self.cwd / pc.KNOWLEDGE_GRAPH)
            fg.serialize(
                self.cwd / "knowledge_graph.nt", format="ntriples", encoding="UTF-8"
            )
            self.ui.info(
                f"N-Triples knowledge graph saved to {self.cwd / 'knowledge_graph.nt'}"
            )
            return self.reasoner.consistency(
                self.cwd / "knowledge_graph.nt", verbose=self.config.verbose
            )
        else:
            return self.reasoner.consistency(
                self.cwd / pc.KNOWLEDGE_GRAPH, verbose=self.config.verbose
            )
    ...
    def _run(self):
        ...
        self.ui.info(
            f"Converting schema file {self.config.paths.schema.absolute()} to OWL format"
        )
        ...
        self.ui.success(
            f"Temporary schema conversion saved to {(self.cwd / pc.ONTOLOGY).absolute()}"
        )

        self.ui.info(
            f"Converting assertions file {self.config.paths.data.absolute()} to OWL format"
        )
        self.reasoner.conversion(
            self.config.paths.data,
            self.cwd / pc.ASSERTIONS,
            format="owl",
            verbose=self.config.verbose,
        )
        self.ui.success(
            f"Temporary assertions conversion saved to {(self.cwd / pc.ASSERTIONS).absolute()}"
        )
        ...
        if self.config.description_logic_profile:
            if self.config.description_logic_profile.lower() in {"alc", "el"}:
                ...
                self.ui.subrule(
                    f"Description Logic Profile Filtering: {profile.upper()}"
                )
                ...
                self.ui.panel(
                    f"{profile.upper()} Profile Ontology",
                    data=[
                ...
                self.reasoner.conversion(self.cwd / pc.ONTOLOGY, self.cwd / pc.ONTOLOGY)

                self.ui.success(
                    "Description logic profile filtering completed successfully"
                )
        ...
        if not self.config.reasoning.satisfiability.filter_unsatisfiable:
            if (
                self.config.reasoning.materialization
                or self.config.reasoning.realization
            ):
                if self.config.interactive_shell:
                    confirm = self.ui.confirm(
                        "Reasoning services are enabled, but unsatisfiable classes and roles are not being filtered. Would you like to run a satisfiability check?"
                    )
                    if confirm:
                        self.ui.info(
                            f"Running satisfiability analysis with {self.config.reasoning.satisfiability.reasoner.upper()} (ROBOT)"
                        )
                        unsatifiable_classes = self.reasoner.satisfiability(
                        ...
                        if not (unsatifiable_classes):
                            self.ui.success(
                                "No unsatisfiable classes or object properties were found"
                            )
                        else:
                            self.ui.warning(
                                f"Reasoning services cannot run safely on ontologies with unsatisfiable classes or roles. Detected unsatisfiable entities: [{unsatifiable_classes}]. Please run with 'filter_unsatisfiable': true."
                            )
                            confirm = self.ui.confirm(
                                "Would you like to remove unsatisfiable URIs now?"
                            )
                            if confirm:
        ...
        if self.config.reasoning.materialization.enabled:
            ...
            self.ui.info(
                f"Running materialization with {self.config.reasoning.materialization.reasoner} (ROBOT)"
            )
            self.ui.info(
                f"Using axiom generators: {self.config.reasoning.materialization.axioms}"
            )
            self.reasoner.materialization(
                self.cwd / pc.ONTOLOGY,
                self.cwd / pc.ONTOLOGY,
                axioms=self.config.reasoning.materialization.axioms,
                verbose=self.config.verbose,
                safety_check=False,
                reasoner=self.config.reasoning.satisfiability.reasoner,
            )
        ...
        self.ui.info(
            "Named individuals that are also declared as classes will be removed"
        )
        self.ui.info(
            "Object properties that are also declared as datatype properties will be removed"
        )
        ...
        for s, p, o in self.ui.progress(
            assertions, "Filtering assertions", total=len(assertions)
        ):
            if (
                (s, RDF.type, OWL.NamedIndividual) in assertions
                and (p, RDF.type, OWL.ObjectProperty) in schema
                and (o, RDF.type, OWL.NamedIndividual) in assertions
            ):
                ...
                if (
                    p not in object_properties
                    and (p, RDF.type, OWL.DatatypeProperty) in schema
                ):
                ...
                (s, RDF.type, OWL.NamedIndividual) in assertions
                and (p == RDF.type)
                and (o, RDF.type, OWL.Class) in schema
            ):
                if s not in individuals and (s, RDF.type, OWL.Class) in schema:
                    null_individuals.add((s, "Also declared as Class"))
                    continue
        ...
        self.ui.panel(
            title="Removed Entities Overview",
            data=[
            ...
            if self.config.interactive_shell:
                ...
                if confirm:
                    if null_individuals:
                        self.ui.list(
                            "Removed Individuals",
                            [f"{a} -> {b}" for a, b in null_individuals],
                        )
                    if null_object_properties:
                        self.ui.list(
                            "Removed Object Properties",
                            [f"{a} -> {b}" for a, b in null_object_properties],
                        )
        ...
        if self.config.split.enabled:
            self.ui.info(
                "Splitting object property assertions into train, validation, and test sets"
            )
            ...
            if self.config.split.transductive:
                ...
                self.ui.info(
                    f"Configured ratios: {self.config.split.train_percent}/{self.config.split.validation_percent}/{self.config.split.test_percent}"
                )
                ...
                self.ui.error(
                    "Inductive splits are not supported yet. The process will now stop"
                )
                self.kill(1)
            ...
            start_usage = mem_mb()
            self.ui.info(
                "Memory cleanup: removing temporary PyKEEN structures from memory"
            )
            del train, valid, test, triples, targets
            gc.collect()
            end_usage = mem_mb()
            self.ui.success(
                f"Memory cleanup completed: freed {start_usage - end_usage:8.2f} MB"
            )

        self.ui.info("Starting serialization of object property assertions")
        self.ui.info(
            f"Object property assertions will be written to {self.cwd / pc.RDF_TRIPLES}"
        )
        ...
        start_usage = mem_mb()
        self.ui.info(
            "Memory cleanup: object property assertions are no longer needed in memory"
        )
        del op_triples
        gc.collect()
        end_usage = mem_mb()
        ...
        self.reasoner.conversion(
            self.cwd / pc.INDIVIDUALS, self.cwd / pc.INDIVIDUALS, format="owl"
        )
        ...
        self.ui.success(
            f"Class assertions serialized to {self.cwd / pc.RDF_CLASS_ASSERTIONS}"
        )
        ...
        if self.config.reasoning.realization.enabled:
            ...
            self.ui.info(
                "Realization is enabled, so the knowledge graph consistency will be checked first"
            )
            self.ui.info(
                f"Merging schema and assertions into {self.cwd / pc.KNOWLEDGE_GRAPH}"
            )
            ...
            if consistent:
                ...
                self.ui.warning(
                    "Knowledge graph is inconsistent; realization cannot proceed"
                )
                if self.config.interactive_shell:
                    confirm = self.ui.confirm(
                        "Would you like to generate an inconsistency justification using Pellet?"
                    )
                    if confirm:
                        self.ui.info(
                            "Running justification on the target knowledge graph"
                        )
                        justification = self.reasoner.justification(
                            input=self.cwd / pc.KNOWLEDGE_GRAPH,
                            output=self.cwd / "justification.owl",
                            verbose=self.config.verbose,
                        )
                        self.ui.list("Inconsistency Justification", justification)
                        self.ui.success(
                            f"Justification saved to {self.cwd / 'justification.ttl'}"
                        )
                        self.kill(0)
        ...
        if self.config.reasoning.realization.enabled:
            ...
            self.ui.info(
                f"Running realization with {self.config.reasoning.realization.reasoner} on the target knowledge base"
            )
            ...
            if self.config.reasoning.realization.reasoner in ["hermit", "elk"]:
                self.ui.info(
                    "Filtering class assertions from the full realization output"
                )
                in_graph = Graph()
                ...
                for s, p, o in self.ui.progress(
                    in_graph, "Filtering class assertions", total=len(in_graph)
                ):
            ...
            self.ui.success(
                f"Realization output saved to {self.cwd / pc.RDF_CLASS_ASSERTIONS}"
            )
        ...
        if self.config.reasoning.modularization.enabled:
            ...
            self.ui.info(
                "Running signature-based schema modularization (PROMPTFACTOR) on the target knowledge base"
            )
            ...
            if self.config.split.enabled:
                self.ui.info(
                    "Reloading object property assertions after machine learning preprocessing"
                )
                op_triples = Graph()
            ...
            self.ui.success(
                f"Schema modularization completed successfully and saved to {self.cwd / pc.ONTOLOGY}"
            )

            start_usage = mem_mb()
            self.ui.info(
                "Memory cleanup: removing the original non-modularized schema from memory"
            )
            del schema
            gc.collect()
            end_usage = mem_mb()
        ...
        if self.config.reasoning.decomposition.tbox:
            ...
            self.reasoner.conversion(
                self.cwd / pc.RDF_TAXONOMY,
                self.cwd / pc.RDF_TAXONOMY,
                format="owl",
                verbose=self.config.verbose,
            )
            self.ui.success(
                f"Taxonomy decomposition saved to {self.cwd / pc.RDF_TAXONOMY}"
            )
            del d_taxonomy
            gc.collect()
        ...
        self.ui.info(
            f"Merging the full knowledge graph into {self.cwd / pc.KNOWLEDGE_GRAPH}"
        )
        ...
        if consistent:
            ...
            if self.config.interactive_shell:
                ...
                if confirm:
                    ...
                    justification = self.reasoner.justification(
                        input=self.cwd / pc.KNOWLEDGE_GRAPH,
                        output=self.cwd / "justification.ttl",
                        verbose=self.config.verbose,
                    )
                    self.ui.list("Inconsistency Justification", justification)
                    self.ui.success(
        ...
        if (
            self.config.post_processing.id_mapping
            or self.config.post_processing.json_conversion
            or self.config.post_processing.tsv_conversion
        ):
        ...
        elapsed = end - start
        self.ui.success(
            f"JDEX pipeline completed in {elapsed:4.2f} seconds. Shutting down cleanly"
        )
